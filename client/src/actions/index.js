...
export function getRecipes() {
	return async function (dispatch) {
		...
		var json = await axios.get('/getRecipes', {});
...
export function getDiets() {
	return async function (dispatch) {
		var info = await axios.get(`/getAllDiets`, {});
		return dispatch({
			type: GET_DIETS,
			payload: info.data,
		});
...
export function postRecipe(payload) {
	return async function (dispatch) {
		try {
			var response = await axios.post(`/postRecipe`, payload);
...
export function getDetails(id) {
	...
	return async function (dispatch) {
		try {
			...
			var detail = await axios.get(`/getDetails/${id}`);
			return dispatch({
